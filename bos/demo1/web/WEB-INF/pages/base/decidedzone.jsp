...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		if(v.length!=1){
            ...
            //然后就是异步查询crm表中的数据,左侧的下拉框是显示还没有被关联的客户的列表,右边下拉框是显示已经此定区关联的客户
			//首先,左侧的下拉列表框
			var url="${pageContext.request.contextPath}/decidedzoneAction_findCustomersAssociation.action";
			//然后发送异步请求来获取数据,这次查询是不需要传输数据的
			$.post(url,{},function (date) {
				//后台传过来的是json数据,是一个数组,解析数组
			...
			//获得当前定区的id
			var ids=v[0].id;
			var url1="${pageContext.request.contextPath}/decidedzoneAction_findCustomersAssociations.action";
			//发送ajax请求,获得已经关联此id的客户的信息
			$.post(url1,{"id":ids},function (date) {
	...
	$(function(){
		...
		$('#grid').datagrid( {
			...
			pagination : true,
			toolbar : toolbar,
			url : "${pageContext.request.contextPath}/decidedzoneAction_findAll.action",
			idField : 'id',
			columns : columns,
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="saveForm" action="${pageContext.request.contextPath}/decidedzoneAction_save.action" method="post" >
					...
					<tr>
						...
						<td>
							<input class="easyui-combobox" name="staff.id"
    							data-options="valueField:'id',textField:'name',url:'${pageContext.request.contextPath}/staffAction_findAjax.action'"/>
						</td>
					</tr>
					<tr height="300">
						<td valign="top">关联分区</td>
						<td>
							<table id="subareaGrid"  class="easyui-datagrid" border="false" style="width:300px;height:300px" data-options="url:'${pageContext.request.contextPath}/subareaAction_findByAjax.action',fitColumns:true,singleSelect:false">
								<thead>  
							        <tr>  
